...
The repaired Earthdata client performed a real `GET` of the existing official
April 2014 GLDAS-2.1 Noah monthly sample from `zindi-gcp` using `~/.netrc`.
Redirects were restricted to HTTPS and normal TLS certificate verification was
left enabled.  Cookies remained in memory only; credential values,
authorization headers, cookies, and response bodies were not logged or
persisted.  The response was HTTP 200 with 25,276,654 bytes.
...
The sanitized `/home/milli/gldas_sample/auth_client_check/verification.json`
confirms `NETCDF4`, product `GLDAS_NOAH025_M.2.1`, April 2014,
`time=1`, `lat=600`, `lon=1440`, and the six required soil-water/SWE/canopy
variables, each with units `kg m-2`.

The earlier 401 remains historical only; it is not used as the current access
status.  The product gate is recorded in
`/home/milli/gldas_sample/auth_client_check/product_gate.json` and is
deliberately more specific than archive coverage alone:

- NASA's monthly 0.25-degree catalogue identifies the selected
  `GLDAS_NOAH025_M.2.1` as the reprocessed main-production stream, generated by
  temporal averaging of the 3-hourly product, with an archive beginning in
  January 2000.
- NASA's GLDAS README documents approximately 3–4 months' latency for the main
  stream versus approximately 1.5 months for the short-rolling Early Product
  stream.
- Organizer discussion 34674 permits non-GRACE GLDAS Noah variables and
  deterministic combinations through source month `t`, while excluding
  GRACE-assimilated/derived products.
- Train ends in 2015-08 and Test ends in 2018-12.  The main-stream values are
  therefore used only as a retrospective historical proxy for this fixed
  replay, not as evidence of ≤1-month current operational availability.

The remaining assumption is explicit: discussion 34674 does not state whether
its GLDAS-2.1 ruling distinguishes main versus Early streams.  This run stays
on the selected main product and enforces the through-`t` causal cutoff.  If
the one-month operational rule is interpreted strictly at each original
prediction date, the main stream's 3–4-month latency would fail that operational
criterion; no Early Product or other dataset is silently substituted.

The sample and conditional historical-proxy gate now pass for the authorized
retrospective experiment, so bounded acquisition and the exact ten-feature
