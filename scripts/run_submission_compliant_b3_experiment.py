...
def _external_access_record(measured: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "external_fit_started": False,
        "product_gate": {
            "status": "pass_conditional_historical_proxy",
            "selected_stream": "main_production",
            "archive_start": "2000-01-01",
            "main_stream_latency": "approximately 3-4 months",
            "historical_proxy_scope": "fixed retrospective Train/Test periods; not current operational availability",
            "organizer_ruling": "discussion 34674 permits non-GRACE GLDAS Noah variables and deterministic combinations through source month t; GRACE-assimilated/derived products are excluded",
            "grace_assimilation": "not_used; GLDAS-2.2/GRACE-derived products remain excluded",
            "remaining_assumption": "discussion 34674 does not explicitly distinguish main versus Early GLDAS-2.1 streams; if the one-month operational rule is applied at each original prediction date, the main stream would not qualify",
            "sample_verification_required_before_bulk": True,
            "official_sources": [
                GLDAS_README_URL,
                GLDAS_METADATA_URL,
                "https://zindi.world/competitions/one-step-ahead-of-drought-forecasting-global-water-storage-challenge/discussions/34674",
            ],
        },
