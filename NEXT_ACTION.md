...
## 2026-09-10 — GLDAS access repaired; product gate resolved as conditional historical proxy

The repaired Earthdata client performed a real `GET` of the existing April
2014 GLDAS-2.1 Noah monthly sample from `zindi-gcp` using `~/.netrc`.  The
working client follows the normal secure redirect path and retains cookies only
in memory; credential values, cookies, authorization headers, and response
bodies are not logged or persisted.  The response was HTTP `200` and
25,276,654 bytes.  The retained sample is
`/home/milli/gldas_sample/auth_client_check/GLDAS_NOAH025_M.A201404.021.nc4`;
its SHA-256 is
`97938ff6bbdc3557fb1c615714aeec0abfa452f000bc15c09a15d2862201ecdb`.

`/home/milli/gldas_sample/auth_client_check/verification.json` records the
sanitized verification: `NETCDF4`, product `GLDAS_NOAH025_M.2.1`, April 2014,
dimensions `time=1`, `lat=600`, `lon=1440`, and all six required variables
(`SoilMoi0_10cm_inst`, `SoilMoi10_40cm_inst`, `SoilMoi40_100cm_inst`,
`SoilMoi100_200cm_inst`, `SWE_inst`, `CanopInt_inst`) with units `kg m-2`.

The old 401 is retained only as historical context in the previous run's
record; it is not the current access status.  The independent product gate is
now recorded in
`/home/milli/gldas_sample/auth_client_check/product_gate.json`:

- NASA's monthly 0.25-degree GLDAS-2.1 catalogue identifies this exact product
  as the reprocessed **main production** stream, generated by temporal
  averaging of the 3-hourly product, with coverage from January 2000 onward.
- NASA's GLDAS README states that the main stream has approximately 3–4 months'
  latency and a long archive; the Early Product stream has approximately
  1.5 months' latency and a short rolling archive.
- Organizer ruling [discussion 34674](https://zindi.world/competitions/one-step-ahead-of-drought-forecasting-global-water-storage-challenge/discussions/34674)
  permits non-GRACE GLDAS Noah variables and deterministic combinations through
  month `t`, and excludes GRACE-assimilated/derived products.
- The challenge's fixed Train/Test dates are historical (Train ends 2015-08;
  Test ends 2018-12).  Therefore the selected main-stream archive is treated
  as a **retrospective historical proxy**, not as a claim that a new main-stream
  value is operationally available within one month of today's acquisition.

The remaining assumption is explicit: ruling 34674 does not distinguish the
main and Early GLDAS-2.1 streams.  This experiment therefore proceeds only as
a historical replay under the ruling's through-`t` constraint.  If the
organizer applies the one-month operational rule strictly to the original
prediction date, the 3–4-month-latency main stream would not qualify; no Early
Product or other dataset is silently substituted here.

With the sample and this conditional historical-proxy gate passed, bounded
acquisition of only the required months/variables may proceed.  The ten-feature
